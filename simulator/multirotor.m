...
% %             disp(plantinput.RotorSpeedsSquared)
    ...
    methods(Access=protected)
        ...
        function new_state = CalcStateNewtonEuler(obj, wrench,dt, is_collision, collision_normal)
            ...
            p_dotdot = wrench(4 : 6) / obj.TotalMass; %F/m
            omega_dot = obj.I_inv * wrench(1 : 3);

            phi_dot = obj.GetEulerRate(); %inertial frame
          
            
            % I assume approximately constant acceleration
            new_state.Acceleration = p_dotdot; % a 
            new_state.AngularAcceleration = omega_dot; % w_dot
