classdef position_controller_fw_MPC_new < pid_controller
    properties
        ...
        Q_u = diag([0.0025 0.0025 0.0025 0.0025 5 5 5 5 1 1 1]);
        Q_t = 40;
        ...
        U0 = zeros(20,11);
        X0 = zeros(21,16); %, get solution TRAJECTORY
        flag = 1;
        mpciter = 1;
        xx = [];
        tiltIntegral = 0
    ...
    methods
        ...
        function [accel_des,rpy_des,tilt_angle_des,V_des] = CalculateControlCommand(obj, mult, pos_des, vel_des, yaw_des, acc_des, time)
            ...
            % 16x1
            state = [V; rpy; rpy_dot;last_rpy_dot;tilt];
            ...
            % eq contraint
            %init var
            roll_ref = deg2rad(23.105);
            omega_ref = [0; 0.057; 0.133];
            
            st = X(:,1);
            g = []; % constraints vector
            g = [g;st-P(1:16)];  %init constraint
            %g = [g;st-P(1:16)];
            ...
            for k = 1:N
                ...
                obj_soft_vel = obj.q_ref(1)*(exp(-3*Velocity_body(1) + 1) + obj.k_x*Velocity_body(1) - obj.c(1))...
                 ...
                 %st(1:3)-P(n_states*k+1:n_states*k+3))' * obj.Q_v * (st(1:3)-P(n_states*k+1:n_states*k+3)
                obj_stateinput = (st(1:3)-P(n_states*k+1:n_states*k+3))' * obj.Q_v * (st(1:3)-P(n_states*k+1:n_states*k+3)) +...
                                   ...
                                   + con' * obj.Q_u * con;
                                   %+ (st(4) - roll_ref)*50*(st(4) - roll_ref)...
                                   %+ (st(7:9) - omega_ref)'*50*(st(7:9) - omega_ref); 
                
                obj_soft_tilt = exp(-0.332*Velocity_body(1)*(st(13)*180/pi)+1.8*(st(13)*180/pi)+(-0.477)*Velocity_body(1)-2.303) +...
                    exp(-0.332*Velocity_body(1)*(st(14)*180/pi)+1.8*(st(14)*180/pi)+(-0.477)*Velocity_body(1)-2.303) +...
                    exp(-0.332*Velocity_body(1)*(st(15)*180/pi)+1.8*(st(15)*180/pi)+(-0.477)*Velocity_body(1)-2.303) +...
                    exp(-0.332*Velocity_body(1)*(st(16)*180/pi)+1.8*(st(16)*180/pi)+(-0.477)*Velocity_body(1)-2.303);
                %obj_soft_tilt = 0;
                ...
                [equ1,equ2,equ3] = obj.solve_equcons(st(1:3),con(1:4),st(7:9),st(14:16));
                %omega_ref = obj.caculate_omega(st(1:3),con(9:11));
                g = [g;st_next - st_RK4_next(1:16)];
            end
...
%             args.lbg(n_states*(N+1)+1:n_states*(N+1)+(N+1)) = -0.5;
%             args.ubg(n_states*(N+1)+1:n_states*(N+1)+(N+1)) = 0.5;
            ...
            args.lbx(5:n_states:n_states*(N+1),1) = -pi/4; args.ubx(5:n_states:n_states*(N+1),1) = 0.3;
            ...
            args.lbx(10:n_states:n_states*(N+1),1) = -pi; args.ubx(10:n_states:n_states*(N+1),1) = pi; %rpy_dot_last in radian
            ...
            args.lbx(13:n_states:n_states*(N+1),1) = -0.122; args.ubx(13:n_states:n_states*(N+1),1) = pi/2; % tilt in degree
            args.lbx(14:n_states:n_states*(N+1),1) = -0.122; args.ubx(14:n_states:n_states*(N+1),1) = pi/2;
            args.lbx(15:n_states:n_states*(N+1),1) = -0.122; args.ubx(15:n_states:n_states*(N+1),1) = pi/2;
            args.lbx(16:n_states:n_states*(N+1),1) = -0.122; args.ubx(16:n_states:n_states*(N+1),1) = pi/2; 

            args.lbx(n_states*(N+1)+1:n_controls:n_states*(N+1)+n_controls*N,1) = 0; args.ubx(n_states*(N+1)+1:n_controls:n_states*(N+1)+n_controls*N,1) = 23; %thrust in Newton calculated from max pitch angle and weight
            args.lbx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = 0; args.ubx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = 23;
            args.lbx(n_states*(N+1)+3:n_controls:n_states*(N+1)+n_controls*N,1) = 0; args.ubx(n_states*(N+1)+3:n_controls:n_states*(N+1)+n_controls*N,1) = 23;
...
%             args.lbx(n_states*(N+1)+5:n_controls:n_states*(N+1)+n_controls*N,1) = -inf; args.ubx(n_states*(N+1)+5:n_controls:n_states*(N+1)+n_controls*N,1) = inf; %tilt_speed in rad
            ...
            args.lbx(n_states*(N+1)+10:n_controls:n_states*(N+1)+n_controls*N,1) = -pi/4; args.ubx(n_states*(N+1)+10:n_controls:n_states*(N+1)+n_controls*N,1) = 0.3;
            args.lbx(n_states*(N+1)+11:n_controls:n_states*(N+1)+n_controls*N,1) = -inf; args.ubx(n_states*(N+1)+11:n_controls:n_states*(N+1)+n_controls*N,1) = inf;

            
            %run mpc
            
            x_state = [Init_Vel ; Init_rpy ; Init_rpy_dot;Init_last_rpy_dot;Init_Tilt];

            u_trim = [zeros(8,1);deg2rad([23.105;0;0])];
...
%               Vx_ref = -200*sin((3/2*pi)+2*pi/43.44*t_predict)*2*pi/43.44; Vy_ref = 200*cos((3/2*pi)+2*pi/43.44*t_predict)*2*pi/43.44; Vz_ref = 0;
        ...
        %equility cons
        function [equ1,equ2,equ3] = solve_equcons(obj,V,T,rpy,tilt)
            g = physics.Gravity(3);
            F_a = obj.CalcAeroForce(V,rpy);
            net_force = obj.CalcAccel(F_a,T,rpy,tilt)* obj.Mass;
            equ1 = net_force(1);
            equ2 = net_force(2) - obj.Mass*norm(V)^2/obj.raduis;
            equ3 = net_force(3) + obj.Mass*g;
        end

        function omega_ref = caculate_omega(obj,V,rpy_MPC)
            yaw_rate = norm(V)/obj.raduis;
            omega_des = [0;0;yaw_rate];

            Euler2PQR_matrix = [1       0           -sin(rpy_MPC(2));
                0       cos(rpy_MPC(1))   sin(rpy_MPC(1))*cos(rpy_MPC(2))
                0       -sin(rpy_MPC(1))   cos(rpy_MPC(1))*cos(rpy_MPC(2))];
            omega_ref = Euler2PQR_matrix * omega_des;
        end
...
% PD controller
body_rate_sp = diag([1,1,1])* (rpyMPC - rpy);
Torque = diag([3,3,3]) * (body_rate_sp - rpy_dot) + diag([0.5,0.5,0.5]) * (last_rpy_dot - rpy_dot);
end
