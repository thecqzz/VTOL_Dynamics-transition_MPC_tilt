classdef position_controller_fw_MPC_new < pid_controller
    ...
    methods
        ...
        function [accel_des,rpy_des,tilt_angle,V_des] = CalculateControlCommand(obj, mult, pos_des, vel_des, yaw_des, acc_des, time)
            ...
            args.lbx(10:n_states:n_states*(N+1),1) = 0; args.ubx(10:n_states:n_states*(N+1),1) = 1.92; % tilt in degree
            ...
            args.lbx(n_states*(N+1)+1:n_controls:n_states*(N+1)+n_controls*N,1) = -0.23; args.ubx(n_states*(N+1)+1:n_controls:n_states*(N+1)+n_controls*N,1) = 125; %thrust in Newton calculated from max pitch angle and weight
%             if Init_Vel(1)<6
...
% 
%             end
            args.lbx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = -0.22; args.ubx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = pi/4; %rpy_MPC in degree
...
%%                 ref tracking
            for k = 1:N %new - set the reference to track
                ...
                if Vx_ref >= 23 % the trajectory end is reached
                    Vx_ref = 23; Vy_ref = 0; Vz_ref = 0;
...
function Torque = CalcTorque(rpyMPC,rpy,rpy_dot,last_rpy_dot) %assuming same thrust and tilt for 4 rotor
